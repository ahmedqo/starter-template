...
function actionColumn({
    ...
    const props = `${csrf ? `csrf="${csrf}"` : ""}${
        scene ? `scene="${scene}"` : ""
    }${patch ? `patch="${patch}"` : ""}${print ? `print="${print}"` : ""}${
        clear ? `clear="${clear}"` : ""
    }`;
    return {
        name: "action",
    ...
    if (opts) {
        field.empty = function () {
            return `<div part="add-new"><span part="empty-data">${$trans(
                "No data found"
            )}</span><a part="add-new-link" href="${opts.link}">${
                opts.text ? opts.text : $trans("Add new")
            }</a></div>`;
        };
    }
}
    ...
    $queryAll("neo-stepper").forEach((stepper) => {
        ...
        stepper.addEventListener("next", (e) => {
            ...
            Neo.Validator.validate(e.detail.step, {
                ...$queryAll("[rules]", e.detail.step).reduce(
                    (carry, item) => {
        ...
        stepper.addEventListener("change:step", (e) => {
            ...
            tabNext.style.display =
                stepper.current < stepper.steps.length - 1 ? "" : "none";
            tabSave.style.display =
                stepper.current === stepper.steps.length - 1 ? "" : "none";
        });
