    ...
    Neo.Component({
        ...
        cycle: {
            mounted() {
                ...
                this.refs.dropdown.addEventListener(
                    "change:expand",
                    this.rules.change
                );
                const wrapper = this.closest("[part=wrapper]");
                wrapper &&
                    wrapper.addEventListener("scroll", this.rules.scroll);
            },
            removed() {
                ...
                const wrapper = this.closest("[part=wrapper]");
                wrapper &&
                    wrapper.removeEventListener("scroll", this.rules.scroll);
            },
        },
        rules: {
            change(e) {
...
function actionColumn({
    ...
    const props = `${csrf ? `csrf="${csrf}"` : ""}${scene ? `scene="${scene}"` : ""
        }${patch ? `patch="${patch}"` : ""}${print ? `print="${print}"` : ""}${clear ? `clear="${clear}"` : ""
        }`;
    return {
        name: "action",
        text: $trans("Actions"),
        ...
        bodyRender (row) {
            ...
            anchor.style =
                "position: relative; width: 2rem; height: 2rem; margin: auto;";
...
function searchable (field, route, colMapper, opts) {
    ...
    field.addEventListener("input", (e) => {
        ...
        timer = setTimeout(async () => {
            const data = await getData(
                route + "?search=" + encodeURIComponent(e.target.query)
            );
    ...
    hidden &&
        field.addEventListener("select", (e) => {
    ...
    if(opts) {
        field.empty = function () {
            return `<div part="add-new"><span part="empty-data">${$trans(
                "No data found"
            )}</span><a part="add-new-link" href="${opts.link}">${opts.text ? opts.text : $trans("Add new")
                }</a></div>`;
        };
    }
}
...
function TableVisualizer ({
    ...
    if(rowClick instanceof Function || rowClick === true)
        dataVisualizer.addEventListener(
            "click:row",
            rowClick === true
                ? function rowClick (e) {
    ...
    if(colClick instanceof Function)
        dataVisualizer.addEventListener("click:col", colClick);
        ...
        stepper.addEventListener("next", (e) => {
            if(stepper.state) stepper.state = false;
                ...
                Neo.Validator.validate(e.detail.step.active, {
                    ...$queryAll("[rules]", e.detail.step.active).reduce(
                        (carry, item) => {
                            carry.rules[item.name] = (
                                item.getAttribute("rules") || ""
                            ).split("|");
                            carry.message.failure[item.name] = JSON.parse(
                                item.getAttribute("errors") || ""
                            );
                            return carry;
                        },
