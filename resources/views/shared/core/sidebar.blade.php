<neo-sidebar id="sidebar" stick>
    <neo-topbar transparent align="start" style="height: 3.5rem" class="shade px-4">
    ...
    <ul class="nav-colors w-full flex flex-col flex-1 p-4">
        ...
        <li class="w-full mib-w-[240px] overflow-hidden">
            <a href="{{ route('views.users.index') }}"
                class="w-full flex flex-wrap gap-2 px-3 py-2 text-start items-center outline-none rounded-x-thin hover:text-x-white hover:bg-x-acent focus:text-x-white focus:bg-x-acent focus-within:text-x-white focus-within:bg-x-acent {{ Neo::matchRoute('users') ? 'bg-x-prime text-x-white' : 'text-x-black' }}">
                <svg class="block w-6 h-6 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                    <path
